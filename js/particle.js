...
!(function(a) {
  ...
  var c = function(a) {
    (this.canvas = a.canvas), (this.g = a.g), (this.particleColor =
      a.options.particleColor), (this.x =
      Math.random() * this.canvas.width), (this.y =
      Math.random() * this.canvas.height), (this.velocity = {
      x: (Math.random() - 0.5) * a.options.velocity,
      y: (Math.random() - 0.5) * a.options.velocity
    });
  };
  return (c.prototype.update = function() {
    (this.x > this.canvas.width + 70 || this.x < -70) &&
      (this.velocity.x = -this.velocity.x), (this.y > this.canvas.height + 70 ||
      this.y < -70) &&
    ...
    (this.canvas = document.createElement("canvas")), this.i.appendChild(
      this.canvas
    ), (this.g = this.canvas.getContext(
      "2d"
    )), (this.canvas.width = this.i.size.width), (this.canvas.height = this.i.size.height), this.l(
      this.i,
      { position: "relative" }
    ), this.l(this.canvas, {
      "z-index": "70",
              ...
              void (this.m = setTimeout(
                function() {
                  ...
                  this.options.interactive &&
                    this.o.push(this.p), requestAnimationFrame(
    ...
    this.options.interactive &&
      (
        ...
        this.canvas.addEventListener(
          "mousemove",
          function(a) {
            (this.p.x = a.clientX - this.canvas.offsetLeft), (this.p.y =
              a.clientY - this.canvas.offsetTop);
          }.bind(this)
        ),
        this.canvas.addEventListener(
          "mouseup",
    ...
    return "fast" === a ? 2 : "slow" === a ? 0.33 : "none" === a ? 0 : 0.66;
